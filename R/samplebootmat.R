...
#'
#' \code{samplebootmat} calls \code{sampleobs} which does the random draws.
#'
...
#'
#' @param ValueID	integer, \code{length = nvalues}, Indicator observation IDs and
#' reference values IDs.
#' @param Value	double, \code{length = nvalues}, Expected values for indicator
#' observations and reference values.
#' @param RefobsID	integer, \code{length = nvalues}, Identifies those elements in
#' \code{ValueID, Value, DistID, mu, sig} that are reference values.
...
#' @param ref.value.code integer, \code{length = 1}, Code used in RefobsID to
...
samplebootmat <- function(ValueID = NULL,
  ...
  if (n.values > 1) {
    ...
    dimnames(bootmat)[[1]] <- list(ValueID)
  }
